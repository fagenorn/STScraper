...
namespace STScraper.Console
{
    public class ProgressBar : IDisposable, IProgress<double>
    {
        ...
        public void Report(string value) { Interlocked.Exchange(ref currentStatus, value); }
        ...
        private void TimerHandler(object state)
        {
            lock (timer)
            {
                ...
                var text               = $"[{new string('#', progressBlockCount)}{new string('-', blockCount - progressBlockCount)}] {percent,3}% {animation[animationIndex++ % animation.Length]} {currentStatus}";
